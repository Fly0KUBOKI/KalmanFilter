...
def analyze():
    ...
    if len(truth_df) == 0:
        ...
        return
    if len(est_df) == 0:
        print("Error: estimation.csv is empty")
        return
    ...
    print(f"\n{'='*60}")
    print(f"ESKF Performance Analysis (with Truth Comparison)")
    print(f"{'='*60}")
    ...
    axes[0, 0].plot(est_df['time'], est_df['px'], label='Est X', linestyle='--', alpha=0.8)
    ...
    axes[0, 1].set_title('Position X Error & Drift')
    ...
    axes[1, 0].plot(est_df['time'], est_df['roll'], label='Est Roll', linestyle='--', alpha=0.8)
    ...
    axes[1, 1].set_title('Roll Error & Drift')
    ...
    axes[2, 0].plot(est_df['time'], est_df['pitch'], label='Est Pitch', linestyle='--', alpha=0.8)
    ...
    axes[2, 1].set_title('Pitch Error & Drift')
    ...
    axes[3, 0].plot(est_df['time'], est_df['yaw'], label='Est Yaw', linestyle='--', alpha=0.8)
    axes[3, 0].set_title('Yaw')
    ...
    axes[3, 1].set_title('Yaw Error & Drift')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'Results', 'analysis_plot.png'), dpi=150)
    print(f"\n{'='*60}")
