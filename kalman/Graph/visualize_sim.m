...
subplot(2,2,1);
plot(true_state(:,1), true_state(:,2), '-k','DisplayName','True'); hold on;
plot(meas.pos(:,1), meas.pos(:,2), '.r','DisplayName','Measured');
plot(x_est(:,1), x_est(:,2), '-b','DisplayName','Kalman');
legend; xlabel('x [m]'); ylabel('y [m]'); axis equal; grid on;
title('Trajectory');
subplot(2,2,2);
plot(t, true_state(:,1), '-k', t, meas.pos(:,1), '.r', t, x_est(:,1), '-b');
legend('True x','Measured x','Kalman x'); xlabel('Time [s]'); ylabel('x [m]'); grid on;
subplot(2,2,3);
plot(t, true_state(:,2), '-k', t, meas.pos(:,2), '.r', t, x_est(:,2), '-b');
...
h_true = plot(true_state(1,1), true_state(1,2), 'ok','MarkerFaceColor','k');
h_meas = plot(meas.pos(1,1), meas.pos(1,2), '.r');
h_est = plot(x_est(1,1), x_est(1,2), 'ob','MarkerFaceColor','b');
