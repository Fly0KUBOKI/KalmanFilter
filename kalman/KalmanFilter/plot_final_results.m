function plot_final_results(vel_results, att_results, pos_results, truth_data, sensor_data, plot_opts)
% PLOT_FINAL_RESULTS - 総合結果プロット
% シミュレーション完了後の詳細な結果表示

fprintf('最終結果をプロットしています...\n');

% plot_opts が渡されていなければ全て有効にする
if nargin < 6 || isempty(plot_opts)
    plot_opts.enable_position = true;
    plot_opts.enable_velocity = true;
    plot_opts.enable_attitude = true;
else
    if ~isfield(plot_opts, 'enable_position'), plot_opts.enable_position = true; end
    if ~isfield(plot_opts, 'enable_velocity'), plot_opts.enable_velocity = true; end
    if ~isfield(plot_opts, 'enable_attitude'), plot_opts.enable_attitude = true; end
end

% 新しい総合結果図を作成（ドック表示）
figure('Name', 'カスケードUKF推定結果総合', 'Position', [50, 50, 1400, 900], 'WindowStyle', 'docked');

% 位置軌跡（メイン） - 有効フラグで制御
if plot_opts.enable_position
    subplot(2, 3, [1, 2, 4, 5]);
    hold on;
...
% 統計情報を表示
fprintf('\n=== 推定精度統計 ===\n');
if plot_opts.enable_velocity && exist('vel_error', 'var') && ~isempty(vel_error)
    fprintf('速度推定 - 平均誤差: %.4f m/s, 最大誤差: %.4f m/s\n', mean(vel_error), max(vel_error));
end
if plot_opts.enable_attitude && exist('yaw_error', 'var') && ~isempty(yaw_error)
