    ...
    void estimate(const char* sensor_type, const cm& innovation, const cm& H, const cm& P_pred) {
        ...
        for (int i = 0; i < H.rows; ++i) {
            for (int j = 0; j < P_pred.cols; ++j) {
                float s = 0.0f;
                for (int k = 0; k < H.cols; ++k) s += H(i,k) * P_pred(k,j);
                tmp(i,j) = s;
            }
        }
        for (int i = 0; i < tmp.rows; ++i) {
            for (int j = 0; j < tmp.rows; ++j) {
                float s = 0.0f;
                for (int k = 0; k < H.cols; ++k) s += tmp(i,k) * H(j,k);
                HPHT(i,j) = s;
            }
        }
            ...
            cm get_R_matrix(const char* sensor_type) const {
                ...
                if (strcmp(sensor_type, "accel") == 0) {
                    R.resize(3,3); for (int i=0;i<3;i++) for(int j=0;j<3;j++) R(i,j) = (i==j) ? R_accel_(i,0) : 0.0f;
                } else if (strcmp(sensor_type, "gyro") == 0) {
                    R.resize(3,3); for (int i=0;i<3;i++) for(int j=0;j<3;j++) R(i,j) = (i==j) ? R_gyro_(i,0) : 0.0f;
                } else if (strcmp(sensor_type, "mag") == 0) {
                    R.resize(3,3); for (int i=0;i<3;i++) for(int j=0;j<3;j++) R(i,j) = (i==j) ? fmaxf(R_mag_(i,0), 0.01f) : 0.0f;
                } else if (strcmp(sensor_type, "gps") == 0) {
                    R.resize(3,3); for (int i=0;i<3;i++) for(int j=0;j<3;j++) R(i,j) = (i==j) ? R_gps_(i,0) : 0.0f;
                } else if (strcmp(sensor_type, "baro") == 0) {
                    R.resize(1,1); R(0,0) = R_baro_;
                }
                return R;
            }
        ...
        public:
            DivergenceGuard()
              : max_allowed_innov_(50.0f), innov_change_ratio_threshold_(2.0f), attenuation_factor_(0.5f),
                max_innov_cap_fraction_(1.0f), min_eigenvalue_factor_(1e-8f), jitter_base_(1e-6f) {}
            ...
            void regularize_covariance(cm& P) {
                int n = P.rows;
                for (int i=0;i<n;i++) for (int j=i+1;j<n;j++) { float avg = (P(i,j)+P(j,i))*0.5f; P(i,j)=avg; P(j,i)=avg; }

                float max_diag = 0.0f; for (int i=0;i<n;i++) max_diag = fmaxf(max_diag, fabsf(P(i,i)));
                if (max_diag < 1e-20f) max_diag = 1e-20f;
                const float eps_abs = 1e-9f; const float eps_rel = min_eigenvalue_factor_; float min_diag = fmaxf(eps_abs, eps_rel * max_diag);
                float jitter = jitter_base_ * max_diag;
                for (int i=0;i<n;i++) { if (P(i,i) < min_diag) P(i,i) = min_diag; else P(i,i) += jitter; }

                if (n == 15) {
                    float caps[15] = {1e6f,1e6f,1e6f,1e4f,1e4f,1e4f,100.0f,100.0f,100.0f,1e2f,1e2f,1e2f,1e-1f,1e-1f,1e-1f};
                    for (int i=0;i<15;i++) P(i,i) = fminf(P(i,i), caps[i]);
                }

                float max_diag2 = 0.0f; float min_diag2 = FLT_MAX;
                for (int i=0;i<n;i++) { float v = fabsf(P(i,i)); max_diag2 = fmaxf(max_diag2, v); min_diag2 = fminf(min_diag2, v); }
                if (max_diag2 < 1e-20f) max_diag2 = 1e-20f; if (min_diag2 < 1e-20f) min_diag2 = 1e-20f;
                float rcond_est = min_diag2 / max_diag2; const float min_rcond = 1e-12f;
                if (rcond_est < min_rcond) { float boost = 1e-8f * max_diag2; for (int i=0;i<n;i++) P(i,i) += boost; }

                const float tiny_thresh = 1e-15f;
                for (int i=0;i<n;i++) for (int j=0;j<n;j++) if (fabsf(P(i,j)) < tiny_thresh) P(i,j) = 0.0f;
            ...
            void clip_state_change(cm& dx) {
                ...
                float pos_norm=0.0f; for(int i=0;i<3;i++) pos_norm += dx(i,0)*dx(i,0); pos_norm = sqrtf(pos_norm); if (pos_norm>max_pos) { float s=max_pos/pos_norm; for(int i=0;i<3;i++) dx(i,0)*=s; }
                float vel_norm=0.0f; for(int i=3;i<6;i++) vel_norm += dx(i,0)*dx(i,0); vel_norm = sqrtf(vel_norm); if (vel_norm>max_vel) { float s=max_vel/vel_norm; for(int i=3;i<6;i++) dx(i,0)*=s; }
                float att_norm=0.0f; for(int i=6;i<9;i++) att_norm += dx(i,0)*dx(i,0); att_norm = sqrtf(att_norm); if (att_norm>max_att) { float s=max_att/att_norm; for(int i=6;i<9;i++) dx(i,0)*=s; }
                float ba_norm=0.0f; for(int i=9;i<12;i++) ba_norm += dx(i,0)*dx(i,0); ba_norm = sqrtf(ba_norm); if (ba_norm>max_ba) { float s=max_ba/ba_norm; for(int i=9;i<12;i++) dx(i,0)*=s; }
                float bg_norm=0.0f; for(int i=12;i<15;i++) bg_norm += dx(i,0)*dx(i,0); bg_norm = sqrtf(bg_norm); if (bg_norm>max_bg) { float s=max_bg/bg_norm; for(int i=12;i<15;i++) dx(i,0)*=s; }
            }
        };
