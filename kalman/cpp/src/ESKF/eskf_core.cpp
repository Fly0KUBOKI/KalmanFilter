...
#include "../../include/KF/kalman_filter_core.hpp"
...
void ESKFCore::integrate_nominal(
    ...
    if (w_dt_norm > threshold) {
        ...
        if (half_angle > 1e-6) {
            ...
            delta_q(0,0) = 1.0 - w_norm_sq/8.0;
            for (int i=0; i<3; ++i) {
                delta_q(i+1,0) = w_dt(i,0) * 0.5 * (1.0 - w_norm_sq/24.0);
            }
        }
...
void ESKFCore::update_accel(
    ...
    if (a_norm < 0.1 || std::abs(a_norm - 9.81) > 3.0) {
        return;
    }
    ...
    Scalar yaw_current = euler_current(2,0);
    ...
    Scalar roll_measured = std::atan2(ay, az) * 180.0 / M_PI;
    Scalar pitch_measured = std::atan2(-ax, std::sqrt(ay*ay + az*az)) * 180.0 / M_PI;
    
    Scalar roll_current = euler_current(0,0);
    Scalar pitch_current = euler_current(1,0);
    
    // スケーリング適用
    Scalar roll_target = roll_current + (roll_measured - roll_current) * scale_factor;
    Scalar pitch_target = pitch_current + (pitch_measured - pitch_current) * scale_factor;
...
void ESKFCore::update_mag(
    ...
    for (int i=0; i<3; ++i) for (int j=0; j<15; ++j) H(i,j) = 0.0;
    ...
    K_out = kf::KalmanFilterCore::compute_kalman_gain<15, 3, Scalar>(P, H, S);
...
void ESKFCore::update_baro(
    ...
    // 気圧->高度変換
    Scalar alt_meas = pressure_to_altitude(pressure);
    Scalar alt_origin = gps_origin(2,0);
    Scalar z_local = alt_meas - alt_origin;
    
    // 観測行列（Z軸のみ）H (1x15)
    cmath_fx::Matrix<1, 15, Scalar> H;
    for (int j=0; j<15; ++j) H(0,j) = (j == 2) ? 1.0 : 0.0;
    
    // イノベーション
    cmath_fx::Matrix<1, 1, Scalar> y;
    y(0,0) = z_local - p(2,0);
    
    // S = H*P*H' + R (1x1)
    auto temp = H * P;  // 1x15
    cmath_fx::Matrix<1, 1, Scalar> S = temp * H.transpose();
    S(0,0) += R_baro;
    
    // カルマンゲイン K = P*H'*S^-1 (15x1)
    K_out = kf::KalmanFilterCore::compute_kalman_gain<15, 1, Scalar>(P, H, S);
    ...
    cmath_fx::Matrix<1, 1, Scalar> R_mat;
    R_mat(0,0) = R_baro;
...
Scalar ESKFCore::pressure_to_altitude(Scalar pressure) {
    ...
    return 44330.0 * (1.0 - std::pow(pressure / P0, 0.1903));
}

void ESKFCore::gps_to_local(const Vector3& gps_pos, const Vector3& origin, Vector3& local_pos) {
    Scalar lat = gps_pos(0,0);
    Scalar lon = gps_pos(1,0);
    Scalar alt = gps_pos(2,0);
    
    Scalar lat0 = origin(0,0);
    Scalar lon0 = origin(1,0);
    Scalar alt0 = origin(2,0);
    
    // 簡易変換（緯度経度->メートル）
    Scalar cos_lat0 = std::cos(lat0 * M_PI / 180.0);
    local_pos(1,0) = (lat - lat0) / 9.0e-6;  // Y (North)
    local_pos(0,0) = (lon - lon0) / (9.0e-6 / cos_lat0);  // X (East)
...
void ESKFCore::predict_covariance(const Matrix15x15& P, const Vector4& q, const Vector3& a_meas, const Vector3& ba,
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            F_theta(i,j) = (i == j ? 1.0 : 0.0) - skew_w(i,j) * dt;
        }
    }
    ...
    for (int i = 0; i < 15; ++i) {
        for (int j = 0; j < 15; ++j) {
            P_new(i,j) = 0.5 * (P_new(i,j) + P_new(j,i));
        }
    }
    
    const Scalar min_var = 1.0e-9;
    for (int i = 0; i < 15; ++i) {
...
void ESKFCore::inject_error_state(Vector3& p, Vector3& v, Vector4& q, Vector3& ba, Vector3& bg, const Vector15& dx) {
    ...
    // 小角度近似: dq ≈ [1; dtheta/2]
    Scalar theta_norm = std::sqrt(dtheta(0,0)*dtheta(0,0) + 
    ...
    if (theta_norm < 1e-6) {
        // 極小角度
        dq(0,0) = 1.0;
        dq(1,0) = dtheta(0,0) * 0.5;
        dq(2,0) = dtheta(1,0) * 0.5;
        dq(3,0) = dtheta(2,0) * 0.5;
    } else {
        // 通常の小角度クォータニオン
        Scalar half_theta = theta_norm * 0.5;
