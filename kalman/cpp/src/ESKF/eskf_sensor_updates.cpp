...
// Helper: Check if any value is NaN (using mxIsNaN for MATLAB compatibility)
#include <mex.h>
...
SensorUpdateResult update_accel_sensor(
    ...
    if (!pre.no_change && !is_nan_any(meas_arr, 3) && !pre.is_outlier) {
        ...
        float w_norm = 0.0f;
        for (int i = 0; i < 3; ++i) {
            float w = static_cast<float>(s->w_body[i]);
            w_norm += w * w;
        }
        ...
        if (w_norm <= 1.5f) {
            ...
            copy_vec(s->prev_accel, meas_arr, 3);
            ...
            // Update accel (direct quaternion update)
...
SensorUpdateResult update_mag_sensor(
    ...
    if (!pre.no_change && !is_nan_any(meas_arr, 3) && !pre.is_outlier) {
        result.should_skip = false;
        copy_vec(s->prev_mag, meas_arr, 3);
        
        // Convert state to float
        Vector<3, float> p_f, v_f, ba_f, bg_f;
        Vector<4, float> q_f;
        Matrix<15, 15, float> P_f;
        ...
        // Get R matrix
        cm R_mat = filter_lib.noise_estimator.get_R_matrix("mag");
        Matrix<3, 3, float> R_mag;
        float mag_min = 0.01f;
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                R_mag(i, j) = (i == j) ? std::fmax(static_cast<float>(R_mat(i, 0)) * 1.5f, mag_min) : 0.0f;
            }
        }
        
        // Mag reference (default: [50, 0, 0])
        Vector<3, float> m_world;
        m_world(0, 0) = 50.0f;
        m_world(1, 0) = 0.0f;
        m_world(2, 0) = 0.0f;
        
        // Update mag
        Matrix<15, 3, float> K_out;
        Vector<15, float> dx_out;
        ESKFCore::update_mag(q_f, P_f, pre.output, m_world, R_mag, K_out, dx_out);
        ...
        if (!post_result.should_skip) {
            // Convert back to double
            for (int i = 0; i < 3; ++i) {
                s->p[i] = static_cast<double>(post_result.p(i, 0));
                s->v[i] = static_cast<double>(post_result.v(i, 0));
                s->ba[i] = static_cast<double>(post_result.ba(i, 0));
                s->bg[i] = static_cast<double>(post_result.bg(i, 0));
            }
            for (int i = 0; i < 4; ++i) {
                s->q[i] = static_cast<double>(post_result.q(i, 0));
            }
            for (int i = 0; i < 15; ++i) {
                for (int j = 0; j < 15; ++j) {
                    s->P[i + j*15] = static_cast<double>(post_result.P(i, j));
                }
            }
            result.updated = true;
        } else {
...
SensorUpdateResult update_baro_sensor(
    ...
    if (std::fabs(pressure - prev_baro) > s->buffer_tolerance) {
        ...
        // Preprocess baro (pressure to altitude)
        double alt_baro = preprocess_baro(pressure);
        
        // Convert state to float
        Vector<3, float> p_f, gps_origin_f;
        Vector<4, float> q_f;
        Matrix<15, 15, float> P_f;
        ...
        // Update baro
        Matrix<15, 1, float> K_out;
        Vector<15, float> dx_out;
        float alt_f = static_cast<float>(alt_baro);
        ESKFCore::update_baro(p_f, P_f, alt_f, gps_origin_f, R_baro, K_out, dx_out);
...
SensorUpdateResult update_gps_sensor(
    ...
    // Convert state to float
    Vector<3, float> p_f, v_f, ba_f, bg_f, gps_origin_f;
    Vector<4, float> q_f;
    Matrix<15, 15, float> P_f;
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            R_gps(i, j) = (i == j) ? static_cast<float>(R_mat(i, 0)) : 0.0f;
        }
    }
    ...
    Matrix<15, 3, float> K_out;
    Vector<15, float> dx_out;
    ESKFCore::update_gps(p_f, v_f, P_f, pre.output, gps_origin_f, R_gps, K_out, dx_out);
