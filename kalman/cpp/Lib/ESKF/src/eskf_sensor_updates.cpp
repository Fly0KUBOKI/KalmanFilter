...
SensorUpdateResult update_baro_sensor(FilterState* s, double pressure, SensorFilterLib& filter_lib) {
    ...
    if (std::fabs(pressure - prev_baro) > s->buffer_tolerance) {
        result.should_skip = false; s->prev_baro = pressure; double alt_baro = sensor::preprocess::baro(pressure);
        ...
        Matrix<15,1,float> K_out; Vector<15,float> dx_out; float alt_f = static_cast<float>(alt_baro); HybridFilterCore::update_baro(p_f, P_f, alt_f, gps_origin_f, R_baro, K_out, dx_out);
...
SensorUpdateResult update_gps_sensor(FilterState* s, double lat, double lon, double alt, SensorFilterLib& filter_lib) {
    ...
    for (int i=0;i<4;++i) q_f(i,0)=s->q[i]; for (int i=0;i<15;++i) for (int j=0;j<15;++j) P_f(i,j)=s->P[i + j*15];
    auto pre = sensor::preprocess::gps(lat, lon, alt, gps_origin_f, s->buffer_tolerance);
    ...
    Matrix<15,3,float> K_out; Vector<15,float> dx_out; HybridFilterCore::update_gps(p_f, v_f, P_f, pre.output, gps_origin_f, R_gps, K_out, dx_out);
    UpdatePostprocessResult post_result = update_state_from_dx(dx_out, p_f, v_f, q_f, ba_f, bg_f, P_f);
