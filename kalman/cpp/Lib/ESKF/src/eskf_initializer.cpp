...
ESKFState* initialize_eskf_state(const ESKFInitializationData& data) {
    ...
    if (data.accel_x && data.accel_y && data.accel_z && N_static > 10) {
        ...
        float psi = 0.0f; if (data.mag_x && data.mag_y && data.mag_z) {
            ...
            Vector<4, float> quat_rp; from_euler_deg(static_cast<float>(phi * 180.0f / MathUtils::PI), static_cast<float>(theta * 180.0f / MathUtils::PI), 0.0f, quat_rp); normalize_quat(quat_rp); float R_rp[9]; quat_to_rotm_array(quat_rp, R_rp);
        ...
        Vector<4,float> quat_final; from_euler_deg(static_cast<float>(phi * 180.0 / MathUtils::PI), static_cast<float>(theta * 180.0 / MathUtils::PI), static_cast<float>(psi * 180.0 / MathUtils::PI), quat_final); normalize_quat(quat_final); q[0]=static_cast<double>(quat_final(0,0)); q[1]=static_cast<double>(quat_final(1,0)); q[2]=static_cast<double>(quat_final(2,0)); q[3]=static_cast<double>(quat_final(3,0));
