...
FilterState* initialize_eskf_state(const ESKFInitializationData& data) {
    ...
    if (data.accel_x && data.accel_y && data.accel_z && N_static > 10) {
        ...
        Vector<4,float> quat_final; from_euler_deg(static_cast<float>(phi * 180.0 / common::math::PI), static_cast<float>(theta * 180.0 / common::math::PI), static_cast<float>(psi * 180.0 / common::math::PI), quat_final); cquat::normalize_quat(quat_final); q[0]=static_cast<float>(quat_final(0,0)); q[1]=static_cast<float>(quat_final(1,0)); q[2]=static_cast<float>(quat_final(2,0)); q[3]=static_cast<float>(quat_final(3,0));
            if (data.pressure) {
                std::vector<float> alt_baro(N_static);
                float alt_mean = 0.0f;
                for (int i = 0; i < N_static; ++i) {
                    float p = static_cast<float>(data.pressure[i]);
                    if (p <= 0.0f) {
                        alt_baro[i] = 0.0f;
                    } else {
                        float ratio = p / 101325.0f;
                        alt_baro[i] = 44330.0f * (1.0f - std::pow(ratio, 1.0f/5.255f));
                    }
                    alt_mean += alt_baro[i];
                }
                alt_mean /= static_cast<float>(N_static);
                float sum_sq = 0.0f;
                for (int i = 0; i < N_static; ++i) {
                    float diff = alt_baro[i] - alt_mean;
                    sum_sq += diff * diff;
                }
                sigma_press = std::sqrt(sum_sq / static_cast<float>(N_static - 1));
                if (sigma_press < 0.1f) sigma_press = 0.1f;
            }
