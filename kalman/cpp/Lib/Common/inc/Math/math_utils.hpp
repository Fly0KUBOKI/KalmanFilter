...
// Namespace-level physical/math constants (declared here, defined in .cpp)
extern const float EPS;
extern const float PI;

// NOTE: geometric angle helpers are defined in geometry.hpp
// (wrap_to_pi, wrap_to_180, angle_difference) — do not redefine them here.
...
inline cm normalize_vector(const cm& v) {
    ...
    if (n < 1.0e-9f) { for (int i = 0; i < result.rows; ++i) result(i,0)=0.0f; }
...
// Backward-compatible wrapper class: calls into free functions above.
// This preserves existing call sites that reference MathUtils::...
class MathUtils {
public:
    static const float EPS;
    static const float PI;

    static float wrap_to_pi(float angle) { return common::math::wrap_to_pi(angle); }
    static float wrap_to_180(float angle) { return common::math::wrap_to_180(angle); }
    static float angle_difference(float a1, float a2) { return common::math::angle_difference(a1,a2); }
    static cm normalize_vector(const cm& v) { return common::math::normalize_vector(v); }
    static cm clip_vector(const cm& v, float max_norm, bool& clipped) { return common::math::clip_vector(v,max_norm,clipped); }
    static double inv_normal_cdf(double p) { return common::math::inv_normal_cdf(p); }
    static double chi2_quantile(int k, double p) { return common::math::chi2_quantile(k,p); }
    static bool is_outlier_chi_square(const cm& innovation, const cm& S, int dof, double alpha = 0.05) { return common::math::is_outlier_chi_square(innovation,S,dof,alpha); }
};
