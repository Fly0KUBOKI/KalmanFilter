...
inline void check_and_reset(ESKFState* s, int k) {
    ...
    if (need_reset) {
        ...
        // Convert results back to internal float state
        for (int i = 0; i < 3; ++i) {
        ...
        for (int i = 0; i < 4; ++i) {
            s->q[i] = static_cast<float>(q_float(i, 0));
        }
        for (int i = 0; i < 15; ++i) {
            for (int j = 0; j < 15; ++j) {
                s->P[i + j*15] = static_cast<float>(P_float(i, j));
            }
        }
    }
}
...
inline void zupt_check_and_update(ESKFState* s, const double* a_meas, const double* w_meas) {
    ...
    if (s->is_stationary) {
        ...
        // Convert back to internal float state
        for (int i = 0; i < 3; ++i) {
            s->v[i] = static_cast<float>(v_out(i, 0));
        }
        for (int i = 0; i < 15; ++i) {
            for (int j = 0; j < 15; ++j) {
                s->P[i + j*15] = static_cast<float>(P_out(i, j));
            }
        }
    }
}
